...
function ExperienceRow({ item, isFirst }: { item: ExperienceItem; isFirst: boolean }) {
  ...
  return (
    <div
      className={
        isFirst
          ? "border-t border-b border-hairline py-4 dark:border-hairline-dark"
          : "border-b border-hairline py-4 dark:border-hairline-dark"
      }
    >
      <div className={`flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1 ${hasBody ? "mb-2" : ""}`}>
      ...
      {item.description.length > 0 && (
        <ul className={`flex flex-col gap-1 ${item.skills.length > 0 ? "mb-2.5" : ""}`}>
          {item.description.map((line) => (
            <li key={line} className="flex gap-2 text-xs font-light leading-[1.65] text-muted dark:text-muted-dark">
              <span aria-hidden="true" className="text-hairline dark:text-hairline-dark">
                —
              </span>
              <span>{line}</span>
            </li>
